...
const state = {
  ...
  // measurements/latest are keyed by device so several devices chart together;
  // config/channels/insights/firmware belong to the active device only.
  data: null,
  loading: false,
...
async function loadData(opts = {}) {
  if (!state.activeDeviceId || !state.authUser) return;
  const seq = ++loadSeq;
...
function buildState() {
  ...
  return {
    ...
    actions: {
      ...
      // Hive audio (issue #71): request a recording, list/play/delete them.
...
function showAuthScreen(node) {
  ...
  if (ui.selectionStrip) ui.selectionStrip.hidden = true;
  ui.sidebar.replaceChildren();
